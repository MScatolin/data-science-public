...
public class EfficientDocument extends Document {
	...
	private void processText()
	{
		...
		for (int i=0; i < tokens.size(); i++){
			...
			// testing how the regex works:
			//System.out.println(piece);
			//System.out.println(piece.length());
			
			//if is not a word, it count as a sentence.
			
			if (isWord(tokens.get(i))){
				
				//in case of the text not have a final ".?!"
				if(i == tokens.size()-1){
				numSentences+=1;
				}
				
				//if not, the methods goes on counting the parameters.
				numWords++;
				
				int pieceSyllables = 0;
				boolean newSyllable = true;
				String vowels = "aeiouy";
				char[] cArray = tokens.get(i).toCharArray();
				for (int j = 0; j < cArray.length; j++)
				{
					
				    if (j == cArray.length-1 && Character.toLowerCase(cArray[j]) == 'e' 
				    		&& newSyllable && pieceSyllables > 0) {
				    	pieceSyllables--;
				    	numSyllables--;
				    	
		            }
				    if (newSyllable && vowels.indexOf(Character.toLowerCase(cArray[j])) >= 0) {
						newSyllable = false;
						pieceSyllables++;
						numSyllables++;
						
					}
					if (vowels.indexOf(Character.toLowerCase(cArray[j])) < 0) {
						newSyllable = true;
