...
public abstract class Graph {
	...
	public int totalDegree(int v){
		int deg;
		deg = getNeighbors(v).size() + getInNeighbors(v).size();
		
		return deg;
	}
	
	public List<Integer> degreeSequence() {
		List<Integer> dsArray = new ArrayList<Integer>();
		int degree;
		for (int i=0 ; i< numVertices; i++){
			degree = totalDegree(i);
			dsArray.add(degree);
		}
		Collections.sort(dsArray, Collections.reverseOrder());
